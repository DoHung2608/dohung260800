...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='aside'>
							<h3 class='aside-title'>Thương hiệu</h3>
							<div class='btn-group-vertical'>
	";
	if(mysqli_num_rows($run_query) > 0){
        $i=1;
			...
			echo "
                        ...
                        <div class='col-md-4 col-xs-6'>
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='add-to-cart'>
										<button pid='$pro_id' id='product' href='#' tabindex='0' class='add-to-cart-btn'><i class='fa fa-shopping-cart'></i>Thêm vào giỏ hàng</button>
									</div>
								</div>
							</div>
			";
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Sản phẩm đã tồn tại trong giỏ</b>
				</div>
			";//not in video
		} else {
			...
			if(mysqli_query($con,$sql)){
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Sản phẩm đã được thêm vào</b>
					</div>
				";
			}
		}
			...
			if (mysqli_num_rows($query) > 0) {
				echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>Sản phẩm đã được thêm vào giỏ hàng tiếp tục mua sắm...!</b>
					</div>";
					exit();
			}
			$sql = "INSERT INTO `cart`
			...
			if (mysqli_query($con,$sql)) {
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Sản phẩm của bạn đã được thêm thành công</b>
					</div>
				";
				exit();
			}
	...
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
            ...
            echo '<div class="cart-summary">
				    <small class="qty">'.$n. ' sản phẩm đã chọn:</small>
		...
		if (mysqli_num_rows($query) > 0) {
			...
			<form method="post" action="login_form.php">
	               ...
	               <table id="cart" class="table table-hover table-condensed" id="">
    				<thead>
						<tr>
							...
							<th style="width:7%" class="text-center">Tổng</th>
				...
				<tfoot>
					...
					<tr>
						<td><a href="store.php" class="btn btn-warning"><i class="fa fa-angle-left"></i>Tiếp tục mua sắm </a></td>
				...
				if (!isset($_SESSION["uid"])) {
					echo '
							...
							<a href="" data-toggle="modal" data-target="#Modal_register" class="btn btn-success">Thanh toán</a></td>
								</tr>
							</tfoot>
						...
						<form action="checkout.php" method="post">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/myfiles/public_html/payment_success.php"/>
									...
									<input type="submit" id="submit" name="login_user_with_product" name="submit" class="btn btn-success" value="Thanh Toán">
									</form></td>
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Sản Phẩm đã bị xóa khỏi cửa hàng</b>
				</div>";
		exit();
	}
}
...
if (isset($_POST["updateCartItem"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-info'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Sản Phẩm được cập nhật</b>
				</div>";
		exit();
	}
}
